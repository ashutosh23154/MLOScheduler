...
MCAB_Scheduler(WifiCoTraceHelper& coHelper, Ptr<Node> sta, Ptr<PacketSocketClient> client)
{
    ...
    auto co_link0 = ComputeChannelOccupancy(coHelper, nodeId, 0 /* LinkId */);
    auto co_link1 = ComputeChannelOccupancy(coHelper, nodeId, 1 /* LinkId */);
    
    /* split-ratio = co_link1:co_link0 */
    double p_tid3 = co_link0 / (co_link0 + co_link1);

    NS_LOG_INFO("p_tid3: " << p_tid3 << " because split-ratio = " << co_link0
                                 << ":" << co_link1);

    /* Adjust flow allocation */
    client->SetAttribute("OptionalTidPr", DoubleValue(p_tid3));
